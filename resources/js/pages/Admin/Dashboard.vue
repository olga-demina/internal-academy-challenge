        ...
        <div v-if="mostPopular" class="rounded-2xl border p-6 shadow-sm">
            ...
            <div class="mt-4 flex items-center gap-6">
                <div class="flex items-center gap-1.5 text-sm">
                    ...
                    <span>
                        <span class="font-semibold">{{ mostPopular.total }}</span>
                        / {{ mostPopular.capacity }} registrations
                    </span>
                </div>

                <div v-if="mostPopular.waiting > 0" class="text-sm text-muted-foreground">
            ...
            <p v-if="mostPopular.waiting > 0" class="mt-3 text-xs text-muted-foreground">
                High demand — consider running this workshop again.
            </p>
        </div>

        <div v-else class="rounded-2xl border border-dashed p-6 text-center text-sm text-muted-foreground">
            No workshops yet.
        </div>
        </div>
    </div>
